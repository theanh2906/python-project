...
class FirebaseStorageManager:
    ...
    def show_credential_upload_dialog(self):
        """Show dialog to upload serviceAccountKey file when none found"""
        try:
            # Show informational dialog first
            result = messagebox.askyesno(
                "Service Account Key Required",
                "No serviceAccountKey file found in the current directory.\n\n"
                "Firebase Storage requires a service account key file to authenticate.\n"
                "The file should be named 'serviceAccountKey.json' or 'serviceAccountKey.b64'\n\n"
                "Would you like to upload your service account key file now?",
                icon="question"
            )
            ...
            # Open file dialog to select credential file
            ...
            if source_ext == '.json':
                target_path = os.path.join(current_dir, "serviceAccountKey.json")
            elif source_ext == '.b64':
                target_path = os.path.join(current_dir, "serviceAccountKey.b64")
            else:
                # Ask user which format they want to save as
                format_choice = messagebox.askyesno(
                    "File Format",
                    "Choose the format to save the credential file:\n\n"
                    "Yes = JSON format (.json)\n"
                    "No = Base64 format (.b64)",
                    icon="question"
                )
                if format_choice:
                    target_path = os.path.join(current_dir, "serviceAccountKey.json")
            ...
            try:
                if target_path.endswith('.json'):
                    ...
                    with open(target_path, 'r') as f:
                        json.load(f)
                elif target_path.endswith('.b64'):
                    # Validate base64 file
                    self.decode_base64_credentials(target_path)
                
                messagebox.showinfo(
                    "Success",
                    f"Service account key file successfully uploaded to:\n{target_path}\n\n"
                    "Firebase will now initialize with the uploaded credentials."
                )
                return True
                ...
                messagebox.showerror(
                    "Invalid File",
                    f"The selected file is not a valid service account key:\n{e}\n\n"
                    "Please select a valid Firebase service account key file."
                )
                return False
                
        except Exception as e:
            messagebox.showerror("Upload Error", f"Failed to upload credential file: {e}")
            return False
    ...
    def init_firebase(self):
        ...
        try:
            ...
            if not firebase_admin._apps:
                # Find credential file in current directory
                credential_file = self.find_credential_file()
                
                if credential_file:
                    try:
                        # Check file extension to determine how to load credentials
                        if credential_file.lower().endswith('.b64'):
                            # Decode base64 encoded credentials
                            credentials_dict = self.decode_base64_credentials(credential_file)
                        ...
                        elif credential_file.lower().endswith('.json'):
                            # Load regular JSON credentials file
                            cred = credentials.Certificate(credential_file)
                            # Create GCS credentials from the same file
                            from google.oauth2 import service_account
                            gcs_credentials = service_account.Credentials.from_service_account_file(credential_file)
                        ...
                        raise Exception(f"Failed to load credentials from {credential_file}: {e}")
                else:
                    # No credential file found - show upload dialog
                    if self.show_credential_upload_dialog():
                        # User uploaded a credential file, retry initialization
